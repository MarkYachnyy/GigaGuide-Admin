...
import ru.vsu.cs.iachnyi_m_a.gigaguide.admin.api.TourReviewAPI
...
class TourReviewRepository():
    ReviewRepository {

    private val tourReviewAPI: TourReviewAPI = Retrofit.Builder().baseUrl("${ServerUtils.SERVER_ADDRESS}/api/reviews/")
        .addConverterFactory(ScalarsConverterFactory.create())
        .addConverterFactory(GsonConverterFactory.create()).build().create(TourReviewAPI::class.java)

    override suspend fun getAllReviews(
        objectId: Int
    ): ReviewSet? {
        var response =
            tourReviewAPI.getAll("Bearer ", objectId).execute()
